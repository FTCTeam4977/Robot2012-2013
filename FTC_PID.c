...
long calcPID(PID &pid, long input)
{
	pid.current = input;
	// P
	...
	pid.output = (long)((pid.Kp*error) + (long)(pid.Ki*pid.errorSum) + (long)(pid.Kd*pid.d));
...
void debugPID(PID &pid)
{
	...
	nxtDisplayString(4, "P res: %2.4f", (float)pid.error*pid.Kp);
}
